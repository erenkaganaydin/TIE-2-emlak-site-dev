<!-- START FOOTER -->
<footer class="first-footer">
    <div class="top-footer bg-white">
        <div class="container">
            <div class="row">
                <div class="col-lg-3 col-md-6">
                    <div class="netabout">
                        <a class="logo" href="index.php">
                            <img alt="netcom" src="images/logo-black.svg?id=1">
                        </a>
                        <p>Başladığımız ilk günden bugüne, kaliteli, anlaşılır ve çözüm odaklı hizmet verme hedefimiz ile binlerce kişiyi doğru alım ve satım işlemi ile bir araya getirdik.</p>
                    </div>
                    <div class="contactus">
                        <ul>
                            <li>
                                <div class="info">
                                    <i aria-hidden="true" class="fa fa-map-marker"></i>
                                    <p class="in-p">İzmir/Alsancak Kıbrs Sehitleri Cad.</p>
                                </div>
                            </li>
                            <li>
                                <div class="info">
                                    <i aria-hidden="true" class="fa fa-phone"></i>
                                    <p class="in-p">+90 555 555 55 55</p>
                                </div>
                            </li>
                            <li>
                                <div class="info">
                                    <i aria-hidden="true" class="fa fa-envelope"></i>
                                    <p class="in-p ti">info@emlaksitem.com</p>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="col-lg-3 col-md-6">
                </div>
                <div class="col-lg-3 col-md-6">
                </div>
                <div class="col-lg-3 col-md-6">
                    <div class="navigation">
                        <h3>Hızlı Menü</h3>
                        <div class="nav-footer">
                            <ul>
                                <li><a href="index.html">Anasayfa</a></li>
                                <li><a href="ilanlar.php">Tüm İlanlar</a></li>
                            </ul>
                            <ul class="nav-right">
                                <li><a href="ilanlar.php?id=4">En Son İlan</a></li>
                                <li><a href="about.php">Hakkımızda</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="second-footer bg-white-3">
        <div class="container">
            <p>2023 © - Tüm Hakları Saklıdır.</p>
            <ul class="netsocials">
                <li><a href="#"><i aria-hidden="true" class="fa fa-facebook"></i></a></li>
                <li><a href="#"><i aria-hidden="true" class="fa fa-twitter"></i></a></li>
                <li><a href="#"><i class="fab fa-instagram"></i></a></li>
                <li><a href="#"><i aria-hidden="true" class="fa fa-youtube"></i></a></li>
            </ul>
        </div>
    </div>
</footer>

<a class="go-up" data-scroll="" href="#wrapper"><i aria-hidden="true" class="fa fa-angle-double-up"></i></a>
<!-- END FOOTER -->

<!-- START PRELOADER -->
<div id="preloader">
    <div id="status">
        <div class="status-mes"></div>
    </div>
</div>
<!-- END PRELOADER -->

<?php include_once "js/genel.php" ?>

</div>
<!-- Wrapper / End -->
</body>

</html>
